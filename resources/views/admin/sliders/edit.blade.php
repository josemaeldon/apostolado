                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <select name="link_type" id="link_type"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    <option value="custom" {{ old('link_type', $slider->linkable_type ? '' : 'custom') == 'custom' ? 'selected' : '' }}>
                                        Link Personalizado
                                    </option>
                                    <option value="page" {{ old('link_type', $slider->linkable_type == 'App\Models\Page' ? 'page' : '') == 'page' ? 'selected' : '' }}>
                                        Página Interna
                                    </option>
                                    <option value="article" {{ old('link_type', $slider->linkable_type == 'App\Models\Article' ? 'article' : '') == 'article' ? 'selected' : '' }}>
                                        Artigo
                                    </option>
                                    <option value="event" {{ old('link_type', $slider->linkable_type == 'App\Models\Event' ? 'event' : '') == 'event' ? 'selected' : '' }}>
                                        Evento
                                    </option>
                                </select>
                                ...
                                <select name="link_id" id="link_id_page"
                                    ...
                                    @foreach($pages as $page)
                                        <option value="{{ $page->id }}" 
                                            {{ old('link_id', $slider->linkable_type == 'App\Models\Page' ? $slider->linkable_id : '') == $page->id ? 'selected' : '' }}>
                                            {{ $page->title }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="link_id" id="link_id_article"
                                    ...
                                    @foreach($articles as $article)
                                        <option value="{{ $article->id }}" 
                                            {{ old('link_id', $slider->linkable_type == 'App\Models\Article' ? $slider->linkable_id : '') == $article->id ? 'selected' : '' }}>
                                            {{ $article->title }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="link_id" id="link_id_event"
                                    ...
                                    @foreach($events as $event)
                                        <option value="{{ $event->id }}" 
                                            {{ old('link_id', $slider->linkable_type == 'App\Models\Event' ? $slider->linkable_id : '') == $event->id ? 'selected' : '' }}>
                                            {{ $event->title }}
                                        </option>
                                    @endforeach
                                </select>
