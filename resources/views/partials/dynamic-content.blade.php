        ...
        <div class="py-12 sm:py-16 lg:py-24" style="background: linear-gradient(to bottom right, {{ $bgColor }}, #ffffff);">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                @if($sectionCards->count() > 0)
                    @php
                        $useMobileCarousel = $sectionCards->count() > 4;
                    ...
                    <div class="relative md:hidden">
                        <div id="{{ $carouselId }}" class="flex gap-4 overflow-x-auto scroll-smooth snap-x snap-mandatory pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                            @foreach($sectionCards as $card)
                                @php
                                    $classes = $card->getCssClasses();
                                @endphp
                                <div class="w-[86%] min-w-[86%] snap-start flex-shrink-0" data-carousel-item>
                                    <div class="{{ $classes['gradient'] }} {{ $classes['border'] }} p-6 rounded-2xl shadow-lg hover:shadow-xl transition" style="{{ $classes['style'] ?? '' }}">
                                        ...
                                        <h4 class="text-xl font-bold {{ $classes['text'] }} mb-3" style="{{ $classes['text_style'] ?? '' }}">{{ $card->title }}</h4>
                        ...
                        <button onclick="scrollCards('{{ $carouselId }}', -1)" class="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 bg-white hover:bg-neutral-100 text-neutral-800 p-2 rounded-full shadow-xl transition z-10" aria-label="Cards anteriores">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                            </svg>
                        </button>
                        <button onclick="scrollCards('{{ $carouselId }}', 1)" class="absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 bg-white hover:bg-neutral-100 text-neutral-800 p-2 rounded-full shadow-xl transition z-10" aria-label="Próximos cards">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="hidden md:grid grid-cols-1 {{ $gridCols }} gap-6 sm:gap-8">
                        @foreach($sectionCards as $card)
                            @php
                                $classes = $card->getCssClasses();
                            @endphp
